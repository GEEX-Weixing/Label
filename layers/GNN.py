...
class GCN(nn.Module):
    def __init__(self, adj, adj_2, input_dim, hidden_dim, num_class):
        super().__init__()
        self.base_gcn = GraphConvSparse(input_dim, 256, adj)
        self.gcn_mean = GraphConvSparse(256, hidden_dim, adj, activation=nn.ELU())
        self.gcn_mean_2 = GraphConvSparse(256, hidden_dim, adj_2, activation=nn.ELU())
        self.classifier = nn.Linear(hidden_dim, num_class)
        self.classifier_2 = nn.Linear(hidden_dim, num_class)
        self.softmax = nn.Softmax()
        self.softmax_2 = nn.Softmax()

    def forward(self, x):
        hidden = self.base_gcn(x)
        embeddings = self.gcn_mean(hidden)
        embeddings_2 = self.gcn_mean_2(hidden)
        logits = self.classifier(embeddings)
        logits_2 = self.classifier_2(embeddings_2)
        logits = self.softmax(logits)
        logits_2 = self.softmax_2(logits_2)

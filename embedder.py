...
class embedder:
    def __init__(self, args):
        ...
        # dataset
        self.data = Dataset(root='data', dataset='cora')[0].to(self.device)
        self.edge_index = self.data.edge_index
        self.edge_index_2 = dropout_adj(self.edge_index, p=0.5)[0]
        self.adj_2 = normalize_adj(np.array(to_dense_adj(self.edge_index_2.cpu(), max_num_nodes=2708).squeeze(0))).to(self.device)
    ...
    def evaluate(self, batch_data, edge_index, edge_index_2, st):

        # Classifier Accuracy
        self.model.eval()
        logits, logits_2, preds = self.model(batch_data, edge_index, edge_index_2)
